<script setup>
import { ref, onMounted, watch } from 'vue'
import { RouterLink } from 'vue-router'
import { fetchList } from '../composables/useApi.js'
import { idToGkId } from '../composables/useGkId.js'
import GkTypeBadge from '../components/GkTypeBadge.vue'
import Pagination from '../components/Pagination.vue'

const page    = ref(1)
const perPage = ref(50)
const loading = ref(false)
const error   = ref(null)
const geokrety = ref([])
const meta    = ref({})
const sortBy  = ref('points')

const sortOptions = [
  { value: 'points',     label: '🏆 Points' },
  { value: 'moves',      label: '🔄 Moves' },
  { value: 'users',      label: '👥 Users' },
  { value: 'countries',  label: '🌍 Countries' },
  { value: 'loves',      label: '❤️ Loves' },
  { value: 'multiplier', label: '✖️ Multiplier' },
]

async function loadGeokrety() {
  loading.value = true
  error.value   = null
  try {
    const { items, meta: m } = await fetchList('/geokrety', {
      page: page.value,
      per_page: perPage.value,
      sort: sortBy.value,
    })
    geokrety.value = items
    meta.value     = m
  } catch (e) {
    error.value = e.message
  } finally {
    loading.value = false
  }
}

onMounted(loadGeokrety)
watch(page, loadGeokrety)

function handleSort() {
  page.value = 1
  loadGeokrety()
}

const fmt = (v) => (v !== null && v !== undefined) ? Number(v).toLocaleString() : '—'
const fmtFloat = (v, d = 2) => (v !== null && v !== undefined) ? Number(v).toFixed(d) : '—'
</script>

<template>
  <div>
    <!-- Breadcrumb -->
    <nav aria-label="breadcrumb" class="mb-3">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><RouterLink to="/">Home</RouterLink></li>
        <li class="breadcrumb-item active" aria-current="page">GeoKrety</li>
      </ol>
    </nav>

    <!-- Header card -->
    <div class="card mb-4 shadow-sm">
      <div class="card-body">
        <h2 class="mb-1">🎁 GeoKrety Database</h2>
        <p class="text-muted mb-0">Browse all GeoKrety items ranked by movement statistics</p>
      </div>
    </div>

    <!-- Sort controls -->
    <div class="mb-3 d-flex gap-2 align-items-center flex-wrap">
      <span class="text-muted small fw-semibold">Sort by:</span>
      <div class="btn-group btn-group-sm" role="group" aria-label="Sort options">
        <template v-for="opt in sortOptions" :key="opt.value">
          <input
            type="radio" class="btn-check" name="gkSort"
            :id="'gksort-' + opt.value"
            :value="opt.value"
            v-model="sortBy"
            @change="handleSort"
          >
          <label class="btn btn-outline-primary" :for="'gksort-' + opt.value">{{ opt.label }}</label>
        </template>
      </div>
      <span class="text-muted ms-auto small">{{ meta.total || 0 }} GeoKrety total</span>
    </div>

    <!-- Loading / Error / Empty -->
    <div v-if="loading && !geokrety.length" class="text-center py-5">
      <div class="spinner-border"></div>
    </div>
    <div v-else-if="error" class="alert alert-danger">{{ error }}</div>
    <div v-else-if="geokrety.length === 0" class="alert alert-info">No GeoKrety found.</div>

    <template v-else>
      <!-- Top pagination -->
      <Pagination v-if="meta.total" :meta="meta" v-model:page="page" class="mb-2" />

      <div class="table-responsive">
        <table class="table table-hover table-sm align-middle border">
          <thead class="table-dark">
            <tr>
              <th style="width: 50px" title="Rank position based on current sort">#</th>
              <th style="width: 90px" title="Public GeoKret ID (GKHEXID)">ID</th>
              <th title="Name of the GeoKret">Name</th>
              <th class="d-none d-md-table-cell" title="GeoKret category type">Type</th>
              <th class="d-none d-lg-table-cell" title="GeoKret owner">Owner</th>
              <th class="d-none d-sm-table-cell" title="Current holder or cache status">Status</th>
              <th class="text-end d-none d-md-table-cell" title="Total number of recorded moves">Moves</th>
              <th class="text-end d-none d-lg-table-cell" title="Number of distinct users who moved it">Users</th>
              <th class="text-end d-none d-lg-table-cell" title="Number of distinct countries visited">Countries</th>
              <th class="text-end" title="Total points generated by this GeoKret">Points</th>
              <th class="text-end d-none d-xl-table-cell" title="Current points multiplier applied to moves">×</th>
              <th class="text-end" title="Number of loves / favourites">❤️</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(gk, idx) in geokrety" :key="gk.gk_id" @click="$router.push(`/geokrety/${gk.gk_id}`)">
              <td class="text-center text-muted fw-semibold small">{{ (page - 1) * perPage + idx + 1 }}</td>
              <td class="small fw-bold">{{ idToGkId(gk.gk_id) }}</td>
              <td>
                <div class="fw-semibold text-truncate" style="max-width: 200px">
                  {{ gk.gk_name || `GK #${gk.gk_id}` }}
                </div>
                <div class="d-md-none small text-muted mt-1">
                  {{ fmt(gk.total_moves) }} moves • {{ fmt(gk.distinct_users) }} users
                </div>
              </td>
              <td class="d-none d-md-table-cell">
                <GkTypeBadge :gk-type="gk.gk_type" :type-name="gk.gk_type_name" />
              </td>
              <td class="d-none d-lg-table-cell">
                <span class="text-truncate d-inline-block" style="max-width: 120px">
                  {{ gk.owner_username || '—' }}
                </span>
              </td>
              <td class="d-none d-sm-table-cell">
                <span v-if="gk.in_cache" class="badge bg-success" title="In cache">🏦 Cache</span>
                <span v-else-if="gk.holder_username" class="badge bg-secondary text-truncate d-inline-block" style="max-width: 100px" :title="`Held by ${gk.holder_username}`">
                  👤 {{ gk.holder_username }}
                </span>
                <span v-else class="text-muted small">—</span>
              </td>
              <td class="text-end fw-bold d-none d-md-table-cell">{{ fmt(gk.total_moves) }}</td>
              <td class="text-end d-none d-lg-table-cell">{{ fmt(gk.distinct_users) }}</td>
              <td class="text-end d-none d-lg-table-cell">{{ fmt(gk.countries_count) }}</td>
              <td class="text-end fw-bold text-success">{{ fmtFloat(gk.total_points_generated, 1) }}</td>
              <td class="text-end text-muted small d-none d-xl-table-cell">{{ fmtFloat(gk.current_multiplier) }}×</td>
              <td class="text-end text-danger">{{ fmt(gk.loves_count) }}</td>
            </tr>
          </tbody>
        </table>
      </div>

      <!-- Bottom pagination -->
      <Pagination v-if="meta.total" :meta="meta" v-model:page="page" class="mt-3" />
    </template>
  </div>
</template>

<style scoped>
tbody tr { cursor: pointer; }
tbody tr:hover { background-color: rgba(0, 123, 255, 0.04); }
</style>
