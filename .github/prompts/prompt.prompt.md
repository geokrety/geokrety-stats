...
- you CAN access the running database to query for any relevant information that can help you in the prompt generation process using #pgsql_query tooling or `psql` (using `.pgpass` or default connection parameters `geokrety` database, `geokrety` user, `geokrety` password, `localhost` host, `5432` port).
