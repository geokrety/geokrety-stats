...
### 1.5 Preferred Component Library: `shadcn/vue`

- **Do not reinvent common UI primitives.** This project uses `shadcn/vue` as the canonical component library and theming system. When implementing or fixing UI components, prefer existing `shadcn/vue` components over custom implementations.
- **Install components via the MCP helper.** Use the `shadcn-vue` MCP integration to add components from the registry instead of copying patterns manually. Example commands (run from the frontend repo root):
...
- **Follow project conventions.** When adding components, respect the project's folder structure (`src/components`, `src/assets/css/globals.css`) and theming via CSS variables / Tailwind config. Prefer shadcn variants and tokens for consistent styling.
- **Reference docs:** https://www.shadcn-vue.com/docs/mcp and https://www.shadcn-vue.com/docs/dark-mode/vite
